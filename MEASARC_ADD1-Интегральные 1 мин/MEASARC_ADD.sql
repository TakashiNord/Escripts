...
begin
  ...
  then
	...
	nID_ARCGINFO:=-1;	   
    select RSDUADMIN.ARC_GINFO.id into nID_ARCGINFO 
      from RSDUADMIN.ARC_GINFO , RSDUADMIN.SYS_GTOPT 
      where RSDUADMIN.SYS_GTOPT.DEFINE_ALIAS='GLT_ANALOG_OPT_INT1' and RSDUADMIN.ARC_GINFO.id_gtopt=RSDUADMIN.SYS_GTOPT.id;    	   

	dbms_output.put_line ('>>> SYS_GTOPT - GLT_ANALOG_OPT_INT1 # '|| nID_ARCGINFO);	  
	...
	for rec in (
     ...
     loop
		dbms_output.put_line ('>>>    ID_SPROFILE: ' || nID_SPROFILE || '   ID_SERVICE: '|| rec.id || '   RETRO_DEPTH: ' || '3600');
        Insert into RSDUADMIN.ARC_SERVICES_ACCESS(ID_SPROFILE, ID_SERVICE, RETRO_DEPTH)
		       Values (nID_SPROFILE, rec.id, 3600);
     end loop;	 
