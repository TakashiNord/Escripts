...
#=========================
    ...
    if (int(RESULT)==0) :
        ...
        os.popen("service ntpd stop").readlines()
        time.sleep(3)
        os.popen("service ntpd start").readlines()
    else:
        output=os.popen("service ntpd status").readlines()
        RESULT = r[0].strip("\n")
...
def ema_time():
    ...
    outs=[]
    for outputstr in os.popen("ntpq -np -4").readlines():
