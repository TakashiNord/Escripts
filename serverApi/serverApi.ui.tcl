# interface generated by SpecTcl version 1.1 from E:/who-broke/serverApi/serverApi.ui
...
proc serverApi_ui {root args} {
	...
	button $base.button#7 \
		-command TOPOraw_network_state_broken_all \
		-text {/../state/ ?broken?-all}
	...
	grid $base.button#6 -in $root	-row 3 -column 5  \
		-sticky nesw
	grid $base.button#12 -in $root	-row 3 -column 6  \
		-sticky nesw
	grid $base.button#7 -in $root	-row 3 -column 7  \
		-sticky nesw
	...
	grid columnconfigure $root 7 -weight 1 -minsize 101
	grid columnconfigure $root 8 -weight 0 -minsize 2
