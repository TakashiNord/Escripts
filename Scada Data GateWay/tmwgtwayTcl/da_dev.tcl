...
# получаем - Направления сбора = id + name + IP + Port
  ...
  set s0 "select t.id, t.name, i.id_dev, i.ip_addr, i.ip_port from da_cat t, da_slave i where i.id_node(+) = t.id and t.id_type in ($s1) and da_get_dirtbl_f (t.id) in ($s2) order by i.ip_addr, t.id " ;
...
# идем по каждому направлению
  foreach {x0} $sl0 {
    ...
    set id  [lindex $x0 0]
    # name
    set idName  [lindex $x0 1]
    # id_dev
    set id_dev  [lindex $x0 2]

    set sf [ format "%s=%-40s=%s:%s" $id  $idName      [lindex $x0 3] [lindex $x0 4]] ;
    set lst [linsert $lst end "${sf}" ] ;
    ...
    # Составляем иерархию + протокол
    ::struct::tree mytreeID
    # начинаем заполнять дерево элементов?  первый элемент (корень)
    ...
    while {$ss} {
        ...
        foreach {id2} [ mytreeID leaves ] {
          ...
          set fah3 [ format $str3 $id_dev $id2 ]
          set yl [ db $fah3 ]
          set ss [ expr $ss + [llength $yl] ]
          foreach {y} $yl {
            set v1 [lindex $y 0] ;
            mytreeID insert $id2 end $v1 ;
          }
        }
    }

    #set sf [ format "mytreeID leaves=%s" [ mytreeID leaves ] ] ;
    #set lst [linsert $lst end "${sf}" ] ;	
    ...
    # Вывод направления сбора
    #ISMS_SERVER_TI01,,,,,,,,,ISMS_SERVER_2404,4,1,13,1,,,"rand(0,100,5000)",,,ISMS_SERVER_TI01decription,
    ...
    set str5 "select dp.id , dp.name, dd.id_proto_point , dd.id_dev , dd.id_parent ,  dd.cvalif , dd.id_type, sys_gtyp.define_alias  \
    ...
    set fah5 [ format $str5 $id ]
    set count 0

    foreach {z} [ db $fah5 ] {
      set did [lindex $z 0]
      set dname [lindex $z 1]
      set dproto [lindex $z 2]
      set dparent [lindex $z 4]
      set dcvalif [lindex $z 5]
      set didtype [lindex $z 6] ; if {$didtype==""} { set didtype 0 ; }
      set dalias [lindex $z 7]
      ...
      set id23 $dparent
	  if {[mytreeID exists $dparent]} { 
	     set anc1 [ mytreeID ancestors $dparent ] ;
      ...
      foreach {z1} [ db $fah58 ] {
        set protoId [lindex $z1 0] ;
        set protoName [lindex $z1 1] ;
        set protoCvalif [lindex $z1 2] ;
	  }
      ...
      set fah5 [ format $str55 $id_dev $dparent ]
      foreach {z3} [ db $fah5 ] {
        set idproto [lindex $z3 5] ;
      }
      ...
      #======================================================================================
        ...
        set flstring [ list "test1"  "test5" "test6" \
                            ...
                            "Hello World"  ]
      #======================================================================================
      # Получаем тип сигнала + В зависимости от типа сигнала установим префикс имени
      set name_prefics "P"
      switch $dalias {
        "GLOBAL_TYPE_ANALOG" {
          set name_prefics "TI"
      ...
      #ISMS_SERVER_TI01,,,,,,,,,ISMS_SERVER_2404,session,ASDU,address,cvalif,,,formula,,,decription,
	...
	set sf [ format "id=%-15s=%-40s  (%10s)"  ${id}  ${idName}  ${count} ]
    puts $sf
