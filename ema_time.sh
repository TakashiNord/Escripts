...
#=========================
    ...
    if (int(RESULT)==0) :
        ...
        os.popen("service ntp stop").readlines()
        time.sleep(3)
        os.popen("service ntp start").readlines()
    else:
        output=os.popen("service ntp status").readlines()
        RESULT = r[0].strip("\n")
        if (RESULT.find("unused")>=0 or RESULT.find("dead")>=0):
            print "service ntp - is dead"
            os.popen("service ntp stop").readlines()
            time.sleep(3)
            os.popen("service ntp start").readlines()
    #
    x = os.popen("ntpd -g").readlines()
    pass
    return 0
...
def check_touch():
    ...
    if (not os.path.exists(touch_file)):
        ...
        return -1
    #output = os.popen("/usr/bin/touch " + touch_file)
    if (touch(touch_file)):
        print "Warning: touch detected read-only mode  ..Signal SS_READONLY"
        if (os.path.exists("/usr/bin/sgtest")):
...
#------ Get free size ------
# 256,7,1,907,SS_RESOURCEFAIL,
# 274,42,1,908,SS_SPACEWARN,
# 81,17,1,908,PHSERV_SPACE,
#

def check_free1():
    global EMA_VAR
    #output = os.popen("df -h $PWD | awk '/[0-9]%/{print $(NF-2)}'")
    #r = output.readlines()
    #output.close()
    #RESULT = r[0].strip("\n")
    path = "/"
    ...
    if (du<=512):
        print "Warning: Size(%s)=%d (Mb)" % (path, du)
        if (os.path.exists("/usr/bin/sgtest")):
            if (EMA_VAR['ss_user_id'] > 0 and EMA_VAR['user_id'] > 0):
                outputstr = "/usr/bin/sgtest %s %s %s %s " % (EMA_VAR["ss_user_id"],EMA_VAR["user_id"],"256", "907")
                output = os.popen(outputstr)
...
def check_free2():
    ...
    for line in p.readlines():
        l.append(re.split(r'\s{0,}',line.strip()))
    p.close()
    for subl in l:
        #print subl
        s=subl[4]
        ...
        #print tV
        if (tV>=98) :
            print "Warning: %s" % subl
            if (os.path.exists("/usr/bin/sgtest")):
                if (EMA_VAR['ss_user_id'] > 0 and EMA_VAR['user_id'] > 0):
                    outputstr = "/usr/bin/sgtest %s %s %s %s " % (EMA_VAR["ss_user_id"],EMA_VAR["user_id"],"274", "908")
                    output = os.popen(outputstr)
    return 0
