...
begin
  ...
  then
	...
	dbms_output.put_line ('>>> Вставка в SYS_GTOPT нового типа # '|| nID_SYSGTOPT);
	Insert into RSDUADMIN.SYS_GTOPT (ID, NAME, ALIAS, ID_GTYPE, DEFINE_ALIAS, INTERVAL, ID_ATYPE)
	       Values (nID_SYSGTOPT, 'Усредненные на границе 10 секунд', 'Уср. 10сек',1, 'GLT_ANALOG_OPT_AVG10SEC', 10,2);
	...
	dbms_output.put_line ('>>> Вставка в ARC_GINFO нового типа архива # '|| nID_ARCGINFO);
	Insert into RSDUADMIN.ARC_GINFO (ID, ID_GTOPT, ID_TYPE, DEPTH, DEPTH_LOCAL, CACHE_SIZE, CACHE_TIMEOUT, FLUSH_INTERVAL, RESTORE_INTERVAL, STACK_INTERVAL, WRITE_MINMAX, RESTORE_TIME, NAME, STATE, DEPTH_PARTITION, RESTORE_TIME_LOCAL)
	       Values (nId_ARCGINFO, nID_SYSGTOPT, 1, 240, 240, 200000, 5, 0, 3600, 300, 1, 720, '&DBArcName', 2, 180, 0);
