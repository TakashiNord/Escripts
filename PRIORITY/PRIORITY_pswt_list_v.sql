...
declare
 ...
 -- 
  ...
  begin
      ...
      --
      rid:=0;
      EXECUTE IMMEDIATE 'Select count(*) from MEAS_LIST ml , MEAS_SRC_CHANNEL msc where msc.ID_OWNLST=ml.ID and ml.id_meas_type IN (SELECT ID FROM pswt_tlist_v) AND is_exdata = 0 and ml.ID_OBJ='||pID INTO rid;
      if rid <=0 then
        return nRetval;
      end if ;
      ...
      FOR recdp in (
            ...
            where msc.ID_OWNLST=ml.ID and ml.id_meas_type IN (SELECT ID FROM pswt_tlist_v) AND is_exdata = 0 and ml.ID_OBJ=pID 
      )
      LOOP
...
begin
    ...
    --
    FOR rec in ( Select ID, ID_PARENT ,ID_TYPE, NAME, ALIAS FROM OBJ_TREE where ID=597 )
    LOOP
        ret1:=GetList (rec.id) ;
        ret2:=GetElem (rec.id) ;
    END LOOP;
